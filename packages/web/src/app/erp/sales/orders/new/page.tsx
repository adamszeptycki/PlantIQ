...
export default function NewSalesOrderPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			// Create sales order
			const orderRes = await fetch("/api/trpc/sales.createSalesOrder", {
				method: "POST",
			...
			const salesOrderId = orderData.result.data.id;

			// Add line items
			for (const item of lineItems) {
				if (item.productId) {
					await fetch("/api/trpc/sales.addSalesOrderLineItem", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						credentials: "include",
						body: JSON.stringify({
	...
	const grandTotal = subtotal + Number.parseFloat(shippingCost) + Number.parseFloat(taxAmount);

	return (
		<div className="min-h-screen bg-slate-900 p-8">
			<div className="max-w-5xl mx-auto">
				<div className="mb-6">
					<h1 className="text-3xl font-bold text-white">Create New Sales Order</h1>
					<p className="text-slate-400 mt-1">
						Create a customer order for fulfillment
					</p>
				</div>
								...
								<div className="flex justify-end">
									<div className="text-right space-y-2">
										...
										<div className="flex items-center gap-4">
											<label className="text-sm text-slate-400">Shipping Cost:</label>
											<input
												type="number"
												step="0.01"
												value={shippingCost}
												onChange={(e) => setShippingCost(e.target.value)}
												className="w-32 px-3 py-1 bg-slate-700 text-white rounded border border-slate-600 focus:border-blue-500 focus:outline-none text-sm"
											/>
										</div>
										<div className="flex items-center gap-4">
											<label className="text-sm text-slate-400">Tax Amount:</label>
											<input
												type="number"
												step="0.01"
												value={taxAmount}
										...
										<div className="text-lg font-semibold text-white pt-2 border-t border-slate-600">
